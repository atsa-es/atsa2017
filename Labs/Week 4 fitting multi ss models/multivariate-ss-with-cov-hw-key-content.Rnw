...
\SweaveOpts{keep.source=TRUE, prefix.string=../figures/KeyCovar-, eps=FALSE, split=FALSE, results=hide}
<<RUNFIRST, echo=FALSE>>=
...
\begin{figure}[htp]
\begin{center}
<<q1-plot2,fig=TRUE,echo=FALSE,width=6,height=6>>=
...
\label{fig.q2a}
\begin{center}
<<hw2-fig1, fig=TRUE, width=6, height=6>>=
...
<<q2-CTemp-set-up, results=verbatim>>=
...
<<q2-CTP-set-up, results=verbatim>>=
...
<<q2-model-compare, results=verbatim>>=
...
\begin{center}
<<hw4-q2-fig1, fig=TRUE, width=6, height=6>>=
...
\Sexpr{round(q3.Tobs$AICc-q2.Temp$AICc, digits=2)} units lower than the model with the effect in the observation equation.
...
\label{fig:q5}
\begin{center}
<<q5-plot1,fig=TRUE,echo=FALSE,width=6,height=6>>=
par(mfrow=c(2,1))
...
abline(h=0)
axis(1,labels=month.abb, at=1:12,las=2,cex.axis=0.75)
title("lm estimates no observation error")
@
\end{center}
...
\begin{center}
<<hw5-fig1, fig=TRUE, width=6, height=6, echo=FALSE>>=
